...
const LeadScreen = ({ navigation }: any) => {
    ...
    return (
      <View style={styles.item}>
        <LeadTabItem
          item={item}
          action={() => {
            if (item?.Sales_Order_Lines__r?.records) {
  ...
  const salesOrderListManager = useCallback(async () => {
    const res = await getSalesOrderList(apiCall);
    return res;
  }, [apiCall]);
    ...
    <SafeAreaView edges={['top']} style={styles.safe}>
      ...
      <View style={styles.screen}>
        ...
        <FlatList
          ...
          ListHeaderComponent={() => <Header dark title="Lead" hide />}
          data={[1, 2]}
